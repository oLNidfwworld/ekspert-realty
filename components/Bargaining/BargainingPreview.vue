...
let center = computed(() => [
            (props.placementObject.PLACE.reduce((a,b) => a + b[0], 0) / (props.placementObject.PLACE.length )),
            (props.placementObject.PLACE.reduce((a,b) => a + b[1], 0) / (props.placementObject.PLACE.length ))
        ]); 
    ...
    <div v-if="placementObject" class="grid gap-10 catalog-item__detail-wrapper">
      <div class="grid gap-10 catalog-item__detail">
        ...
        <div> 
          ...
          <p class="font-semibold">МКР Трубицыно Участок {{placementObject.NAME}}</p>
                ...
                <div class="relative">
                  ...
                  <transition name="share-appear">
                    <div  v-if="isShareButtonActive" class="flex z-[2] bshadow top-[60px] absolute left-0 right-0 p-3 hover:b bg-grey-light rounded-[6px] w-fit m-auto">
                      <Share :link="`https://t.me/share/url?url=${currUrl()}&text=Участок ${placementObject.NAME}`" titol='Поделиться в Telegram'>
                      ...
                      <Share :link="`http://vk.com/share.php?url=${currUrl()}&title=Участок ${placementObject.NAME}`" titol='Поделиться в VK'>
                      ...
                      <Share :link="`https://api.whatsapp.com/send?text=Участок ${placementObject.NAME} ${currUrl()}`" titol='Поделиться в WhatsApp'>
                        <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 30 30" fill="none">
                      ...
                      <Share :link="`https://connect.ok.ru/offer?url=${currUrl()}&title=Участок ${placementObject.NAME}`" titol='Поделиться в Одноклассниках'>
                        <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 30 30" fill="none">
                      ...
                      <Share :link="`viber://forward?text=Участок ${placementObject.NAME} ${currUrl()}&utm_source=share2`" titol='Поделиться в Viber'>
                        <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 30 30" fill="none">
