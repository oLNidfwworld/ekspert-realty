    ...
    <transition name="left-right-slide" mode="in-out" v-show="showMenu">
      <ul class="quick-menu__items-wrapper" v-show="showMenu">
        <li class="quick-menu__item" v-for="(link, index) in menu" :key="index">
          <NuxtLink :to="{ path: link.url }">{{ link.name }} <template v-if="link.count">{{ link.count }}</template></NuxtLink>
        </li>
      </ul>
    </transition>
  </div>
...
.left-right-slide-enter-active li,
.left-right-slide-leave-active li {
  opacity: 1;
  transition: all 0.3s ease-in-out;
}
.left-right-slide-enter-from li:nth-child(even),
.left-right-slide-leave-to li:nth-child(even) {
  transform: translateX(-50%);
  opacity: 0;
}
.left-right-slide-enter-from li:nth-child(odd),
.left-right-slide-leave-to li:nth-child(odd) {
  transform: translateX(50%);
  opacity: 0;
}
</style>
