...
const { data: product, pending, error, refresh } = await useAsyncData(
    () => useApiFetch(`/Catalog/${route.params.id}/`)
)
const myPhone = ref('')
...
const callbackMe = async () => {
  if (myPhone.value.length > 15) {
    const {data: res} = await useApiFetchWithRefresh('/Send/', {
      method: 'POST',
      params: {
        action: 'callback',
        phone: myPhone.value
    ...
    if(res.response === 'success') {
      return alert('Спасибо за заявку, с вами скоро свяжутся')
    }
  }

    return alert('Проверьте корректность телефона')
}
useHead({
